...
describe('the finish control speaks to the woman pressing it', () => {
  it('the PAUSE screen offers סיימי ושמרי, not סיים ושמור', () => {
    setGender('female');
    const said = texts(mount(<CardioLiveView {...liveProps} paused />)).join(' ');
  ...
  it('and so does the CONFIRMATION behind it', () => {
    setGender('female');
    const said = texts(mount(<CardioLiveView {...liveProps} paused confirmEnd />)).join(' ');
    expect(said).toContain('סיימי ושמרי');
    expect(said).not.toContain('סיים ושמור');
  });
...
describe('nothing on the run is set too small to read', () => {
  ...
  const STATES: Array<[string, React.ReactElement]> = [
    ['running', <CardioLiveView key="a" {...liveProps} />],
    ['a kilometre just logged (the split pill)', <CardioLiveView key="b" {...liveProps} splits={SPLITS} />],
    ['paused', <CardioLiveView key="c" {...liveProps} splits={SPLITS} paused />],
    ['the end sheet', <CardioLiveView key="d" {...liveProps} splits={SPLITS} paused confirmEnd />],
    ['the kilometre moment', <CardioLiveView key="e" {...liveProps} splits={SPLITS} kmMoment={SPLITS[1]} />],
  ];
  ...
  it('the three he named by hand — קרדיו, the band, the kilometre label', () => {
    setGender('female');
    const sizes = typeSizes(mount(<CardioLiveView {...liveProps} />));
    ...
    expect(sizeOf('ק״מ')).toBeGreaterThanOrEqual(13); // the kilometre label under its figure
  });
  ...
  it('the saved stage’s readout labels agree with the live one’s', () => {
    setGender('female');
    const live = typeSizes(mount(<CardioLiveView {...liveProps} />)).find((x) => x.text === 'ק״מ')?.size;
    // `KM` on the live row and `זמן` on the done row are the same slot in the same instrument.
