...
describe('the bounds — every one of them', () => {
  ...
  it('⛔ 3 · a swap must STRICTLY reduce the changes — equal is not better', () => {
    // Already grouped: nothing to gain, so nothing moves, and the array comes back by IDENTITY so a
    // caller can tell that no decision was made.
    const grouped = [at('cable_row'), at('cable_curl'), at('db_curl'), at('db_row')];
...
describe('what it must never change', () => {
  ...
  it('⚠️ an unchanged plan comes back by IDENTITY', () => {
     ...
     */
    const already: CoachPlan = { v: 2, sessions: [{ name: 'A', blocks: [at('cable_row'), at('cable_curl'), at('db_curl'), at('db_row')] }] };
