...
describe('the source itself carries no v4 decision input', () => {
  ...
  it('the cold-start load never reads experience or age', () => {
    const s = src('domain/startingLoad.ts');
    // Comments explain WHY they are gone; code must not reference them.
    const code = s.split('\n').filter((l) => !l.trim().startsWith('*') && !l.trim().startsWith('//')).join('\n');
    expect(code).not.toMatch(/profile\.(experience|age)/);
    expect(code).not.toMatch(/EXP_FACTOR|ageLoadFactor/);
  });

  it('the model never forks on a goal or a declared volume', () => {
    const code = src('data/api/fixtureModel.ts')
      .split('\n').filter((l) => !l.trim().startsWith('*') && !l.trim().startsWith('//') && !l.trim().startsWith('/*')).join('\n');
    expect(code).not.toMatch(/WeeklyVolume|profile\.volume/);
    expect(code).not.toMatch(/goal ===|goal:\s*Goal/);
    expect(code).not.toMatch(/canonicalEngineId|engineSlotId/);
    expect(code).not.toMatch(/displayWeekNumber/); // no calendar gate on what the engine says
  });
