...
export interface CoachWeekDraft {
  days: { name: string; lifts: { ex: string; sets: number; pair?: boolean }[] }[];
  /**
...
export function readCoachWeek(json: unknown): CoachWeekDraft | null {
  ...
  for (const d of days) {
    ...
    const rows: { ex: string; sets: number }[] = [];
    for (const l of lifts) {
      ...
      rows.push({ ex, sets, ...(pair ? { pair: true } : {}) });
    }
    out.push({ name, lifts: rows });
  }
...
export function draftFromCoachWeek(
   ...
   */
  return materializeTemplate({ id: opts.id ?? 'coach', days }, (name) => name) as Program;
}
