 ...
 * i18next's `context`, so `t('home.greeting')` resolves `home.greeting_female` for a woman
 * and falls back to the base key when no feminine form exists (English authors none). No
 * screen passes anything: the hook re-renders when the gender is picked, and the whole app
 * changes person at once. A caller that genuinely needs the ungendered string can pass its
 * own `context` and override it.
 */
