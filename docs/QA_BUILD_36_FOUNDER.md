...
| C.10 | 2.4d Rest · learned | **What "learned rest" means is not clear at all.** What actually happens when rests are shortened? And **adding +15 s shows nothing**, so the learning looks one-directional. | **the +15 half is DONE — it was a real bug and his words named it exactly.** Two correct rules met: `restTotalS` never grows when she presses +15 (the fill-forward law, which is what makes the ring sweep visibly), and the elapsed rest was read as `total − remaining` off that same frozen total. So every second she ADDED was invisible to the measurement — she stretched her rest, the beat reported the old duration, her median did not move, and the one direction the feature could not learn was the one she had just demonstrated with her thumb. `restedSeconds(total, remaining, extra)` now, tested and watched to fail. **The COPY half — what "learned rest" means in words — is still his.** |
