...
| **F-21** | **A raise needs one rep of headroom on the worst set** (`RAISE_HEADROOM_REPS`, S-22b, 2026-09-10 — measured). S-22 raised whenever every set met Tlo, including when the worst set landed EXACTLY on it; traced, that is the oscillation the board printed for a month — set 4 reaches 8 → raise → set 4 falls to 6–7 → hold → stall → back off → 8 → raise. A clear at the edge is the edge. With one rep of margin required, and the 8–12 default band and B-10 that shipped with it, the board went 38.5% → 63.5% in band, mean miss 1.47 → 0.82 reps, set 1 32.8% → 73.5%; alone on the old band and seed it is worth ~38.5% → 44%. One rep, not two: two would hold a lift that is ready on most grids. A clear at the edge is still a clear (S-24 never reads it as a stall). | no — it **gates** when S-22 may move the load |
