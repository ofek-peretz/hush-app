...
| **B-11** | **The day-one rest follows the PRESCRIPTION, not only the lift** (`REST_BAND_FACTORS`, 2026-09-10). B-4 read one fact — is this a compound — and the founder's line for it is *"rest matches the work"*. It did not: a set of five and a set of fifteen on the same barbell were both given 150 seconds. Since 2026-09-07 the model writes a rep RANGE per seat from her own words (`Slot.repBand`), and the engine already obeys it in the fold and in the prescription; the clock was the one thing that never followed. The band's FLOOR is the intensity — a prescription starting at five reps is near-maximal and needs minutes, one starting at twelve is metabolic — so the tier bootstrap is multiplied by 1.5 / 1.25 / 1.0 / 0.8 at floors of ≤5 / ≤7 / 8–11 / ≥12. Applied to the BOOTSTRAP only: the moment she has `MIN_REST_SAMPLES` of her own (F-17) her measured median is the whole answer. The same number reaches the standalone wrist, which until now fell back to one flat 90 seconds for every lift in the week. | no — it **shapes** a day-one rest, replaced by her own median within three sets |
...
| **F-23** | **The lightest bell — 4 kg** (`KETTLEBELL_KG`, 2026-09-10). The floor under every `kettlebell` load, the same room-fact as `BAR_KG` (S-55) and `FIXED_BAR_KG` (F-19): a bell is one cast object and the smallest a home set holds is 4 kg. The family arrived with the home-gym room and its ladder is coarse and fixed — 8 · 12 · 16 · 20 · 24 — so B-6's increment for it is 4, and her performed grid (F-2) refines it the moment she owns something off the ladder. Without a floor `snapDown` would prescribe a 2 kg kettlebell, a thing that does not exist, and no loop can correct downward out of a floor (S-55b). | yes — it **floors** every kettlebell load |
